...
create table if not exists public.content_items (
  ...
  type text not null check (type in ('Ideia', 'Roteiro', 'Artigo', 'Carrossel', 'Vídeo', 'Shorts', 'Reels', 'Email', 'Copy', 'Anúncio')),
  title text not null,
  description text,
  category text not null,
...
create table if not exists public.trends (
  ...
  competition_level text not null check (competition_level in ('baixa', 'média', 'alta')),
...
create table if not exists public.content_ideas (
  ...
  funnel_stage text not null check (funnel_stage in ('topo', 'meio', 'fundo', 'pós-venda')),
  status text not null default 'rascunho' check (status in ('rascunho', 'em_análise', 'aprovada', 'em_produção', 'produzida', 'publicada', 'arquivada', 'descartada')),
...
create table if not exists public.prompt_templates (
  ...
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  name text not null,
  category text not null check (category in ('roteiro', 'título', 'gancho', 'carrossel', 'artigo', 'email', 'anúncio', 'whatsapp', 'descrição youtube', 'seo')),
  description text,
...
create table if not exists public.ai_generations (
  ...
  status text not null default 'aguardando' check (status in ('aguardando', 'processando', 'concluído', 'erro')),
  error_message text,
  created_by uuid references auth.users(id) on delete set null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
...
create trigger touch_trends_updated_at before update on public.trends for each row execute function public.touch_updated_at();
...
create trigger touch_competitor_insights_updated_at before update on public.competitor_insights for each row execute function public.touch_updated_at();
...
create trigger touch_idea_scores_updated_at before update on public.idea_scores for each row execute function public.touch_updated_at();
create trigger touch_idea_sources_updated_at before update on public.idea_sources for each row execute function public.touch_updated_at();
create trigger touch_ai_providers_updated_at before update on public.ai_providers for each row execute function public.touch_updated_at();
create trigger touch_prompt_templates_updated_at before update on public.prompt_templates for each row execute function public.touch_updated_at();
create trigger touch_ai_generations_updated_at before update on public.ai_generations for each row execute function public.touch_updated_at();
create trigger touch_ai_generation_jobs_updated_at before update on public.ai_generation_jobs for each row execute function public.touch_updated_at();
create trigger touch_ai_agents_updated_at before update on public.ai_agents for each row execute function public.touch_updated_at();
create trigger touch_voice_providers_updated_at before update on public.voice_providers for each row execute function public.touch_updated_at();
create trigger touch_voices_updated_at before update on public.voices for each row execute function public.touch_updated_at();
create trigger touch_image_providers_updated_at before update on public.image_providers for each row execute function public.touch_updated_at();
create trigger touch_video_projects_updated_at before update on public.video_projects for each row execute function public.touch_updated_at();
create trigger touch_video_scenes_updated_at before update on public.video_scenes for each row execute function public.touch_updated_at();
create trigger touch_magic_video_jobs_updated_at before update on public.magic_video_jobs for each row execute function public.touch_updated_at();
create trigger touch_viral_clip_jobs_updated_at before update on public.viral_clip_jobs for each row execute function public.touch_updated_at();
...
create trigger touch_production_plans_updated_at before update on public.production_plans for each row execute function public.touch_updated_at();
create trigger touch_ai_video_providers_updated_at before update on public.ai_video_providers for each row execute function public.touch_updated_at();
create trigger touch_image_to_video_jobs_updated_at before update on public.image_to_video_jobs for each row execute function public.touch_updated_at();
create trigger touch_text_to_video_jobs_updated_at before update on public.text_to_video_jobs for each row execute function public.touch_updated_at();
create trigger touch_premium_templates_updated_at before update on public.premium_templates for each row execute function public.touch_updated_at();
...
create trigger audit_trends_change after insert or update or delete on public.trends for each row execute function public.audit_row_change();
create trigger audit_competitors_change after insert or update or delete on public.competitors for each row execute function public.audit_row_change();
create trigger audit_competitor_insights_change after insert or update or delete on public.competitor_insights for each row execute function public.audit_row_change();
create trigger audit_content_ideas_change after insert or update or delete on public.content_ideas for each row execute function public.audit_row_change();
create trigger audit_idea_scores_change after insert or update or delete on public.idea_scores for each row execute function public.audit_row_change();
create trigger audit_idea_sources_change after insert or update or delete on public.idea_sources for each row execute function public.audit_row_change();
create trigger audit_idea_events_change after insert or update or delete on public.idea_events for each row execute function public.audit_row_change();
create trigger audit_ai_providers_change after insert or update or delete on public.ai_providers for each row execute function public.audit_row_change();
create trigger audit_prompt_templates_change after insert or update or delete on public.prompt_templates for each row execute function public.audit_row_change();
create trigger audit_ai_generations_change after insert or update or delete on public.ai_generations for each row execute function public.audit_row_change();
create trigger audit_ai_generation_jobs_change after insert or update or delete on public.ai_generation_jobs for each row execute function public.audit_row_change();
create trigger audit_ai_agents_change after insert or update or delete on public.ai_agents for each row execute function public.audit_row_change();
create trigger audit_playground_messages_change after insert or update or delete on public.playground_messages for each row execute function public.audit_row_change();
create trigger audit_ai_credit_usage_change after insert or update or delete on public.ai_credit_usage for each row execute function public.audit_row_change();
create trigger audit_voice_providers_change after insert or update or delete on public.voice_providers for each row execute function public.audit_row_change();
create trigger audit_voices_change after insert or update or delete on public.voices for each row execute function public.audit_row_change();
create trigger audit_audio_generations_change after insert or update or delete on public.audio_generations for each row execute function public.audit_row_change();
create trigger audit_image_providers_change after insert or update or delete on public.image_providers for each row execute function public.audit_row_change();
create trigger audit_image_generations_change after insert or update or delete on public.image_generations for each row execute function public.audit_row_change();
create trigger audit_media_assets_change after insert or update or delete on public.media_assets for each row execute function public.audit_row_change();
create trigger audit_video_projects_change after insert or update or delete on public.video_projects for each row execute function public.audit_row_change();
create trigger audit_video_scenes_change after insert or update or delete on public.video_scenes for each row execute function public.audit_row_change();
create trigger audit_subtitle_segments_change after insert or update or delete on public.subtitle_segments for each row execute function public.audit_row_change();
create trigger audit_music_tracks_change after insert or update or delete on public.music_tracks for each row execute function public.audit_row_change();
create trigger audit_video_renders_change after insert or update or delete on public.video_renders for each row execute function public.audit_row_change();
create trigger audit_media_usage_logs_change after insert or update or delete on public.media_usage_logs for each row execute function public.audit_row_change();
create trigger audit_visual_style_presets_change after insert or update or delete on public.visual_style_presets for each row execute function public.audit_row_change();
create trigger audit_video_effects_change after insert or update or delete on public.video_effects for each row execute function public.audit_row_change();
create trigger audit_video_ai_providers_change after insert or update or delete on public.video_ai_providers for each row execute function public.audit_row_change();
create trigger audit_image_animations_change after insert or update or delete on public.image_animations for each row execute function public.audit_row_change();
create trigger audit_subtitle_styles_change after insert or update or delete on public.subtitle_styles for each row execute function public.audit_row_change();
create trigger audit_audio_settings_change after insert or update or delete on public.audio_settings for each row execute function public.audit_row_change();
create trigger audit_thumbnail_generations_change after insert or update or delete on public.thumbnail_generations for each row execute function public.audit_row_change();
create trigger audit_video_versions_change after insert or update or delete on public.video_versions for each row execute function public.audit_row_change();
create trigger audit_magic_templates_change after insert or update or delete on public.magic_templates for each row execute function public.audit_row_change();
create trigger audit_magic_video_jobs_change after insert or update or delete on public.magic_video_jobs for each row execute function public.audit_row_change();
create trigger audit_source_videos_change after insert or update or delete on public.source_videos for each row execute function public.audit_row_change();
create trigger audit_video_transcripts_change after insert or update or delete on public.video_transcripts for each row execute function public.audit_row_change();
create trigger audit_viral_clip_jobs_change after insert or update or delete on public.viral_clip_jobs for each row execute function public.audit_row_change();
create trigger audit_viral_moments_change after insert or update or delete on public.viral_moments for each row execute function public.audit_row_change();
create trigger audit_viral_clips_change after insert or update or delete on public.viral_clips for each row execute function public.audit_row_change();
create trigger audit_channels_change after insert or update or delete on public.channels for each row execute function public.audit_row_change();
create trigger audit_channel_templates_change after insert or update or delete on public.channel_templates for each row execute function public.audit_row_change();
create trigger audit_content_calendar_change after insert or update or delete on public.content_calendar for each row execute function public.audit_row_change();
create trigger audit_production_plans_change after insert or update or delete on public.production_plans for each row execute function public.audit_row_change();
create trigger audit_bulk_jobs_change after insert or update or delete on public.bulk_jobs for each row execute function public.audit_row_change();
create trigger audit_channel_goals_change after insert or update or delete on public.channel_goals for each row execute function public.audit_row_change();
create trigger audit_channel_permissions_change after insert or update or delete on public.channel_permissions for each row execute function public.audit_row_change();
create trigger audit_operation_notifications_change after insert or update or delete on public.operation_notifications for each row execute function public.audit_row_change();
create trigger audit_ai_video_providers_change after insert or update or delete on public.ai_video_providers for each row execute function public.audit_row_change();
create trigger audit_image_to_video_jobs_change after insert or update or delete on public.image_to_video_jobs for each row execute function public.audit_row_change();
create trigger audit_text_to_video_jobs_change after insert or update or delete on public.text_to_video_jobs for each row execute function public.audit_row_change();
create trigger audit_intro_outro_generations_change after insert or update or delete on public.intro_outro_generations for each row execute function public.audit_row_change();
create trigger audit_talking_character_jobs_change after insert or update or delete on public.talking_character_jobs for each row execute function public.audit_row_change();
create trigger audit_ai_video_assets_change after insert or update or delete on public.ai_video_assets for each row execute function public.audit_row_change();
create trigger audit_content_factories_change after insert or update or delete on public.content_factories for each row execute function public.audit_row_change();
create trigger audit_production_rules_change after insert or update or delete on public.production_rules for each row execute function public.audit_row_change();
create trigger audit_factory_schedules_change after insert or update or delete on public.factory_schedules for each row execute function public.audit_row_change();
create trigger audit_content_series_change after insert or update or delete on public.content_series for each row execute function public.audit_row_change();
create trigger audit_factory_queue_jobs_change after insert or update or delete on public.factory_queue_jobs for each row execute function public.audit_row_change();
create trigger audit_review_queue_items_change after insert or update or delete on public.review_queue_items for each row execute function public.audit_row_change();
create trigger audit_factory_alerts_change after insert or update or delete on public.factory_alerts for each row execute function public.audit_row_change();
create trigger audit_backup_jobs_change after insert or update or delete on public.backup_jobs for each row execute function public.audit_row_change();
create trigger audit_data_retention_policies_change after insert or update or delete on public.data_retention_policies for each row execute function public.audit_row_change();
create trigger audit_security_events_change after insert or update or delete on public.security_events for each row execute function public.audit_row_change();
create trigger audit_rate_limits_change after insert or update or delete on public.rate_limits for each row execute function public.audit_row_change();
create trigger audit_error_logs_change after insert or update or delete on public.error_logs for each row execute function public.audit_row_change();
create trigger audit_data_requests_change after insert or update or delete on public.data_requests for each row execute function public.audit_row_change();
...
insert into public.niches (name, is_default, active) values
  ...
  ('Inteligência Artificial', true, true),
  ('E-commerce', true, true),
  ('Imóveis', true, true),
  ('Seguros', true, true),
  ('Proteção Veicular', true, true),
  ('Jurídico', true, true),
  ('Turismo', true, true)
