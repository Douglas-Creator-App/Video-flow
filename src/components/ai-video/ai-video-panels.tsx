...
import { aiVideoAssets, aiVideoProviders, projects, voices } from "@/lib/mock-data";
import { estimateAiVideoCost, motionPrompts } from "@/lib/ai-video/ai-video-pipeline";
import type { AiVideoProviderType, CameraMotion, VideoAspectRatio } from "@/lib/types";

type AiVideoQueuedResult = {
  status: "queued";
  job_id: string;
  provider: string;
...
export function AiVideoStudio() {
  ...
  const [provider, setProvider] = useState<AiVideoProviderType>("runway");
  ...
  const [result, setResult] = useState<AiVideoQueuedResult | null>(null);
    ...
    <div className="grid gap-4 xl:grid-cols-[1fr_380px]">
      <Card className="border-primary/25 bg-[linear-gradient(135deg,rgb(24_24_24/.96),rgb(10_10_10/.98))]">
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="Provider"><SelectField value={provider} onChange={(e) => setProvider(e.target.value as AiVideoProviderType)}>{aiVideoProviders.filter((p) => p.provider !== "mock").map((p) => <option key={p.id} value={p.provider}>{p.name}</option>)}</SelectField></Field>
...
export function TalkingScenesStudio() {
  ...
  async function generate() {
    ...
    const response = await fetch("/api/ai-video/jobs", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ workspace_id: workspaceId, type: "talking_character", provider, speech_text: speechText, input_image_url: "https://example.com/character.png" }) });
    setResult(await response.json());
  }
  return (
    <div className="grid gap-4 xl:grid-cols-[1fr_380px]">
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><UserRound className="h-4 w-4 text-primary" />Personagem falante</CardTitle><CardDescription>Imagem, fala, voz, estilo e duracao para gerar mini video.</CardDescription></CardHeader><CardContent className="grid gap-4 md:grid-cols-2"><Field label="Imagem/personagem"><Input defaultValue="https://example.com/character.png" /></Field><Field label="Provider"><SelectField value={provider} onChange={(e) => setProvider(e.target.value as AiVideoProviderType)}>{aiVideoProviders.filter((p) => p.provider !== "mock").map((p) => <option key={p.id} value={p.provider}>{p.name}</option>)}</SelectField></Field><Field label="Voz"><SelectField>{voices.map((v) => <option key={v.id}>{v.name}</option>)}</SelectField></Field><Field label="Duracao"><SelectField><option>8s</option><option>10s</option></SelectField></Field><Field label="Descricao"><Input defaultValue="Narrador documental com expressao seria" /></Field><Field label="Fala"><Input value={speechText} onChange={(e) => setSpeechText(e.target.value)} /></Field><Button onClick={generate} className="gap-2 md:col-span-2"><Play className="h-4 w-4" />Gerar personagem falando</Button></CardContent></Card>
      <ResultCard result={result} />
    </div>
  );
}
...
export function AiVideoHistory() {
  ...
  useEffect(() => {
    if (!workspaceId) return;
    async function load() {
      const response = await fetch(`/api/ai-video/jobs?workspace_id=${encodeURIComponent(workspaceId)}`, { cache: "no-store" });
      const data = await response.json();
      if (!response.ok) {
        setMessage(data.error ?? "Falha ao carregar historico real.");
        return;
      }
      setJobs(data.jobs ?? []);
      setMessage(data.jobs?.length ? "" : "Nenhum job real de AI Video neste workspace.");
    }
    void load();
  }, [workspaceId]);

  return <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{message ? <Card><CardContent className="p-4 text-sm text-muted-foreground">{message}</CardContent></Card> : null}{jobs.map((job) => <Card key={String(job.id)}><CardHeader><Badge className="w-fit">{String(job.status)}</Badge><CardTitle className="mt-2">{String((job.payload as { prompt?: string })?.prompt ?? job.type ?? "Video IA")}</CardTitle><CardDescription>{String((job.payload as { provider?: string })?.provider ?? "provider")} - {String((job.payload as { required_credits?: number })?.required_credits ?? 0)} creditos</CardDescription></CardHeader><CardContent><Button asChild variant="outline" className="w-full gap-2"><Link href="/app/queue"><Download className="h-4 w-4" />Abrir fila</Link></Button></CardContent></Card>)}</div>;
}
...
function ResultCard({ result }: { result: AiVideoQueuedResult | null }) {
  return <Card className="border-primary/15"><CardHeader><CardTitle>Resultado</CardTitle><CardDescription>Jobs rodam em fila e exigem provider real. Sem API key/adapter, o worker falha com erro claro.</CardDescription></CardHeader><CardContent className="space-y-3">{result ? <><div className="grid aspect-video place-items-center rounded-md bg-black/70"><Film className="h-8 w-8 text-primary" /></div><Badge>{result.status}</Badge><p className="text-sm text-muted-foreground">Job {result.job_id} enviado para {result.provider}. Custo estimado: {result.cost_estimate} creditos.</p><div className="rounded-md border border-primary/20 bg-primary/5 p-3 text-xs text-muted-foreground">{result.warning ?? "Acompanhe o progresso na fila."}</div><Button asChild className="w-full"><Link href="/app/queue">Abrir fila</Link></Button></> : <p className="rounded-md border border-white/5 bg-secondary/40 p-3 text-sm text-muted-foreground">Nenhum job gerado ainda.</p>}</CardContent></Card>;
}
