...
export async function POST(request: NextRequest) {
  ...
  await requireRateLimit({ workspaceId: workspace.workspaceId!, userId: user.id, feature: "openai_tts", route: "/api/ai/tts" });
  const usage = await canUseFeature(workspace.workspaceId ?? "ws_1", "generate_voice");
